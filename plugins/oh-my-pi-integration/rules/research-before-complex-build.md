...
condition: ["complex (functionality|feature|logic|module|system)", "implement(ing)? (a )?(complex|non-trivial|advanced)", "research.*(implement|library|dependenc|existing)", "existing (reliable )?(implement|solution|library)|proven (library|implementation)", "which (library|dependency|implementation)|pick (a )?(library|dependency)", "build from scratch|reinvent"]
scope: ["text", "thinking"]
---
