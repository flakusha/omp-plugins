...
condition: ["\\bjson\\b|deserializ|untyped|dynamic (data|object)|\\bunknown\\b|parsed (object|data)|api (response|payload)|config (file|object)|wire format", "shape (validation|check)|validate the shape|field (presence|type|shape)", "\\btypeof\\b|instanceof|type guard|estimate (the )?type|\\bas\\b cast|satisfies|structural (check|validation)"]
scope: ["text", "thinking"]
---
