...
condition: ["(?=[\\s\\S]*\\btry\\b|\\bcatch\\b|\\bfinally\\b|try/catch|try-catch)(?=[\\s\\S]*error handling|error path|error-handl)(?=[\\s\\S]*exception|\\bthrow\\b|\\bpanic\\b|onError)(?=[\\s\\S]*propagat(e|ion)?|rethrow)(?=[\\s\\S]*swallow(ed)? (error|exception))(?=[\\s\\S]*catch.*ignore|empty catch)"]
scope: ["text", "thinking"]
---
