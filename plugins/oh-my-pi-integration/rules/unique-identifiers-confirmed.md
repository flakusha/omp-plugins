...
condition: ["unique id|uuid|guid|idempoten|primary key|identity|collision|enumerable", "create (a )?(record|row|entity|resource)|insert|add (a )?new|generate (an )?id|upsert", "api|database|db|table|collection|data store"]
scope: ["text", "thinking"]
---
