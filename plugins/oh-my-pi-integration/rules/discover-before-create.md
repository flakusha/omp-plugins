...
condition: ["new (function|feature|functionality|module|util|helper|api)", "implement|create (a )?(function|util|helper|module)", "add.*(function|util|helper)", "similar.*(exist|implemented)|already (exists|implemented|wired)", "duplicat(ed|e)? (function|code|implementation)|near-identical", "reuse|shared module|DRY|one source of truth", "discover|search (the )?(code|repo)|call graph|dependency graph|symbol (search|lookup)"]
scope: ["text", "thinking"]
---
