...
condition: ["performance|optimiz|fast|speed|slow|latency|lookup|search (is|in) (an? )?(array|set|dict|map)|dynamic programming", "dict|map|set|array|hash|list (vs|or) (dict|set)|lookup (table|structure)|memoiz|\\bDP\\b", "memory (constrained|bounded|limit)|allocation|uncontrolled|naive (solution|approach)|simple (solution|approach)"]
scope: ["text", "thinking"]
---
