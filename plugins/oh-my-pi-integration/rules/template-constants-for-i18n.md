...
condition: ["(?=[\\s\\S]*handlebar|mustache|template (engine|syntax)|\\{\\{|\\{placeholder)(?=[\\s\\S]*i18n|internationaliz|translation|localiz)(?=[\\s\\S]*constant.*(template|html|string)|placeholder.*(key|constant))(?=[\\s\\S]*hardcod.*(string|text|label|button))(?=[\\s\\S]*user-visible (string|text|label))"]
scope: ["text", "thinking"]
---
