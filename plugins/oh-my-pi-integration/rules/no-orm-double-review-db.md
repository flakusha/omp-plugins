...
condition: ["no orm|without (an? )?orm|raw (sql|query|db)|plain (sql|query)|direct (db|database) (access|query|driver)", "sqlite3|\\bpg\\b|mysql|odbc|jdbc|driver|hand-?written (sql|query)", "db|database|access (command|query)|no (query builder|abstraction|orm)"]
scope: ["text", "thinking"]
---
